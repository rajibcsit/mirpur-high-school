...
<section class="relative z-20 -mt-7 max-w-7xl mx-auto px-4">
    <div class="news-ticker rounded-2xl overflow-hidden shadow-xl">
        <div class="ticker-label">LATEST NEWS</div>
        <div class="ticker-window">
            @if($latestNews->isNotEmpty())
                <div class="ticker-track">
                    @foreach($latestNews as $item)
                        <a href="{{ $item->external_url ?: route('news.show', $item->slug) }}" @if($item->external_url) target="_blank" rel="noopener noreferrer" @endif>
                            <span>{{ $item->title }}</span>
                            <small>{{ $item->published_at?->format('d M Y') ?: $item->created_at?->format('d M Y') }}</small>
                        </a>
                    @endforeach
                    @foreach($latestNews as $item)
                        <a href="{{ $item->external_url ?: route('news.show', $item->slug) }}" @if($item->external_url) target="_blank" rel="noopener noreferrer" @endif aria-hidden="true" tabindex="-1">
                            <span>{{ $item->title }}</span>
                            <small>{{ $item->published_at?->format('d M Y') ?: $item->created_at?->format('d M Y') }}</small>
                        </a>
                    @endforeach
            ...
            @else
                <div class="px-6 py-4 text-sm font-semibold text-gray-500">No latest news published yet.</div>
            @endif
        </div>
...
{{-- Teachers --}}<section class="home-faculty-section text-white py-20"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex justify-between items-end mb-10"><div><span class="section-eyebrow text-gold">OUR FACULTY</span><h2 class="section-title text-white">Meet our <span>teachers.</span></h2></div><a href="{{ route('teachers.index') }}" class="text-white font-bold">View faculty →</a></div><div class="grid sm:grid-cols-2 md:grid-cols-4 gap-5">@foreach($teachers as $teacher)<div class="teacher-card reveal-scale">@if($teacher->photo_path)<img src="{{ asset('storage/'.$teacher->photo_path) }}" alt="{{ $teacher->name }}">@else<div class="teacher-placeholder">{{ strtoupper(substr($teacher->name,0,1)) }}</div>@endif<div class="p-5"><h3 class="font-bold text-lg">{{ $teacher->name }}</h3><p class="text-gold text-sm mt-1">{{ $teacher->designation }}</p><p class="text-white/50 text-xs mt-1">{{ $teacher->subject }}</p></div></div>@endforeach</div></div></section>
